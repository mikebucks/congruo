...
console.log(
  `usages: ${extract.usages.length} (ds: ${extract.usages.filter((u) => u.definitionRef).length}, styled: ${extract.usages.filter((u) => u.kind === "styled-element").length})`,
);
console.log(
  `tokens: ${extract.tokens.length}, diagnostics: ${extract.diagnostics.length}`,
);
...
const sample = extract.definitions
  .slice(0, 5)
  .map((d) => `${d.name}(${d.props.length}p)`);
